...
function SchemaCanvas({ schema, onUpdate }) {
  ...
  const [connecting, setConnecting] = useState(null); // null | "waiting" | elementId
  ...
  function addElement() {
    ...
    const el = { id: genId(), label: newEl.label.trim(), icon: newEl.icon, color: newEl.color, x: 60 + Math.random() * 180, y: 60 + Math.random() * 120 };
  ...
  function handleElPointerDown(e, id) {
    if (connecting) return; // in connect mode, ignore drag
    e.stopPropagation();
    const rect = canvasRef.current.getBoundingClientRect();
    const { x: cx, y: cy } = getClientPos(e);
    const el = elements.find(el => el.id === id);
    setDragging({ id, offsetX: cx - rect.left - el.x, offsetY: cy - rect.top - el.y });
    setSelected(id);
  }
  ...
  function handleCanvasPointerMove(e) {
    ...
    const rect = canvasRef.current.getBoundingClientRect();
    const { x: cx, y: cy } = getClientPos(e);
    const x = Math.max(0, Math.min(cx - rect.left - dragging.offsetX, rect.width - 72));
    const y = Math.max(0, Math.min(cy - rect.top - dragging.offsetY, rect.height - 62));
    onUpdate({ ...schema, elements: elements.map(el => el.id === dragging.id ? { ...el, x, y } : el) });
  }

  function handleCanvasPointerUp() { setDragging(null); }
  function handleCanvasClick() { if (!connecting) setSelected(null); }

  function getCenter(el) { return { x: el.x + 36, y: el.y + 26 }; }

  const selEl = elements.find(e => e.id === selected);
  const connectMode = !!connecting;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <button
          onClick={() => setConnecting(connecting ? null : "waiting")}
          style={{ ...S.btnGhost, fontSize: 13, padding: "7px 14px", border: connectMode ? "2px solid #6366f1" : undefined, color: connectMode ? "#6366f1" : undefined, background: connectMode ? "#eef2ff" : undefined }}
        >
          🔗 {connectMode ? (connecting === "waiting" ? "Clica no 1º elemento…" : "Clica no 2º elemento…") : "Ligar"}
        </button>
        {connectMode && <button onClick={() => setConnecting(null)} style={{ ...S.btnGhost, fontSize: 13, padding: "7px 10px" }}>✕ Cancelar</button>}
      ...
      {addingEl && (
        <div style={{ ...S.card, padding: 0 }}>
          ...
          <div style={{ padding: "14px 18px", display: "flex", flexDirection: "column", gap: 12 }}>
            <div><label style={S.label}>Nome *</label><input value={newEl.label} onChange={e => setNewEl(n => ({ ...n, label: e.target.value }))} onKeyDown={e => e.key === "Enter" && addElement()} style={S.input} placeholder="Ex: Quadro, Tomada A3…" autoFocus /></div>
            <div>
      ...
      {editEl && (
        <div style={{ ...S.card, padding: 0 }}>
          ...
          <div style={{ padding: "14px 18px", display: "flex", flexDirection: "column", gap: 12 }}>
            <div><label style={S.label}>Nome</label><input value={editEl.label} onChange={e => setEditEl(el => ({ ...el, label: e.target.value }))} style={S.input} autoFocus /></div>
            <div>
              <label style={S.label}>Ícone</label>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
      ...
      <div
        ...
        style={{ position: "relative", width: "100%", height: 400, background: "#f8fafc", borderRadius: 14, border: `2px solid ${connectMode ? "#6366f1" : "#e2e8f0"}`, overflow: "hidden", cursor: connectMode ? "crosshair" : "default", touchAction: "none" }}
      >
        {/* Grid */}
        ...
        {elements.map(el => {
          ...
          const isSource = connecting === el.id;
          return (
            <div
              key={el.id}
              onMouseDown={e => handleElPointerDown(e, el.id)}
              onTouchStart={e => handleElPointerDown(e, el.id)}
              onClick={e => handleElClick(e, el.id)}
              style={{
                position: "absolute", left: el.x, top: el.y, width: 72,
                display: "flex", flexDirection: "column", alignItems: "center", gap: 4,
                cursor: connectMode ? "pointer" : dragging?.id === el.id ? "grabbing" : "grab",
                userSelect: "none", zIndex: isSelected || isSource ? 10 : 1,
              }}
            >
              <div style={{
                width: 54, height: 54, borderRadius: 12,
                background: el.color + "22",
                border: `2.5px solid ${isSource ? "#f59e0b" : isSelected ? el.color : el.color + "77"}`,
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 26,
                boxShadow: isSource ? `0 0 0 4px #fef3c7` : isSelected ? `0 0 0 3px ${el.color}44` : "none",
                transition: "box-shadow 0.2s, border 0.2s",
              }}>
                {el.icon}
              </div>
              <span style={{
                fontFamily: "'Sora',sans-serif", fontSize: 10, fontWeight: 700, color: "#1e293b",
                textAlign: "center", background: "rgba(255,255,255,0.92)", padding: "2px 6px",
                borderRadius: 5, maxWidth: 72, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
              }}>{el.label}</span>
              {isSource && <span style={{ fontSize: 9, color: "#f59e0b", fontFamily: "'Sora',sans-serif", fontWeight: 700 }}>ORIGEM</span>}
            </div>
          );
        })}
        ...
        <div style={{ position: "absolute", bottom: 8, right: 10, fontFamily: "'Sora',sans-serif", fontSize: 10, color: "#94a3b8" }}>
          Arrasta · Seleciona · ✕ nas linhas para apagar
        </div>
      </div>
